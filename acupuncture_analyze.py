...
def fit_forearm_alpha(input_image,x_min,y_min):
...
def fit_forearm_beta(input_image,x_min,y_max):
	hsv_image = cv2.cvtColor(input_image,cv2.COLOR_BGR2HSV)
...
def fit_forearm_gamma(input_image,x_max,y_min):
	hsv_image = cv2.cvtColor(input_image,cv2.COLOR_BGR2HSV)
	#Get (y_max,x_min) HSV values
	#20 is offset. Otherwise, it is detecting the rectangle box
	x_max = x_max-60
	...
	while(matched==False):
		...
		if(abs(previous_S - pixel[1])>30):
			matched = True
		y_min+=1
	cv2.putText(input_image,"Beta",(x_max-30,y_min-30), cv2.FONT_HERSHEY_SIMPLEX,0.8, (255,0,0), 1, cv2.LINE_AA)
	cv2.circle(input_image,(x_max,y_min),5,(255,0,0),-1)
	#cv2.imshow("Detector",input_image)
	return input_image,x_max,y_min

def fit_forearm_delta(input_image,x_max,y_max):
	hsv_image = cv2.cvtColor(input_image,cv2.COLOR_BGR2HSV)
	#Get (y_max,x_min) HSV values
	#20 is offset. Otherwise, it is detecting the rectangle box
	x_max = x_max-60
	...
	while(matched==False):
		...
		if(abs(previous_S - pixel[1])>30):
			matched = True
		y_max-=1
	cv2.putText(input_image,"Delta",(x_max-30,y_max+30), cv2.FONT_HERSHEY_SIMPLEX,0.8, (255,0,0), 1, cv2.LINE_AA)
...
def main():
	...
	#Active shrinking 
	raw_image,alpha_x,alpha_y=fit_forearm_alpha(raw_image,x1,y1)
	raw_image,beta_x,beta_y=fit_forearm_beta(raw_image,x1,y2)
	raw_image,gamma_x,gamma_y=fit_forearm_gamma(raw_image,x2,y1)
	raw_image,delta_x,delta_y=fit_forearm_delta(raw_image,x2,y2)
	...
	while(i<=12):
		...
		if(i == 0):
			...
			cv2.putText(raw_image,"0",(x_shift+10,midy-10),cv2.FONT_HERSHEY_SIMPLEX,1,(178,102,255), 2, cv2.LINE_AA)
			
			cv2.circle(raw_image,(x_shift,mid_1),5,(178,102,255),-1)
			cv2.putText(raw_image,"2",(x_shift+10,mid_1-10),cv2.FONT_HERSHEY_SIMPLEX,1,(178,102,255), 2, cv2.LINE_AA)

			cv2.circle(raw_image,(x_shift,mid_2),5,(178,102,255),-1)
			cv2.putText(raw_image,"1",(x_shift+10,mid_2-10),cv2.FONT_HERSHEY_SIMPLEX,1,(178,102,255), 2, cv2.LINE_AA)
		i+=1
